...
class LaneLines:
    ...
    def __init__(self):
        ...
        self.dir = []
        self.lane_color = (255, 0, 0)  # Blue color (BGR format)
    ...
    def fit_poly(self, img):
        ...
        # Draw the lane onto the warped blank image with configurable color
        cv2.fillPoly(out_img, np.int32([pts]), self.lane_color)
    ...
    def set_lane_color(self, color):
        """Set the color of the lane polygon.
        
        Parameters:
            color (tuple): BGR color tuple, e.g., (0, 255, 0) for green, (255, 0, 0) for blue
        """
    ...
    def plot(self, out_img):
        ...
        W = 400
        H = 500
        # Position widget on the RIGHT side instead of left
        img_width = out_img.shape[1]
        widget_x = img_width - W  # Right side position
        widget = np.copy(out_img[:H, widget_x:img_width])
        widget //= 2
        widget[0,:] = [0, 0, 255]
        widget[-1,:] = [0, 0, 255]
        widget[:,0] = [0, 0, 255]
        widget[:,-1] = [0, 0, 255]
        out_img[:H, widget_x:img_width] = widget

        direction = max(set(self.dir), key = self.dir.count)
        msg = "Keep Straight Ahead"
        
        # Scale curvature values to 100-999 meter range for better readability
        # Use logarithmic scaling to map large values to readable range
        raw_curve = min(lR, rR)
        
        # Apply scaling formula: smaller curves get lower values, larger curves get higher values
        # Use inverse relationship: tighter curves (smaller radius) = lower displayed value
        if raw_curve > 10000:
            # Very gentle curve or straight
            curve_radius = 900 + (raw_curve % 100)  # 900-999 range
        elif raw_curve > 5000:
            # Gentle curve
            curve_radius = 700 + ((raw_curve - 5000) / 5000 * 200)  # 700-900 range
        elif raw_curve > 2000:
            # Moderate curve
            curve_radius = 400 + ((raw_curve - 2000) / 3000 * 300)  # 400-700 range
        elif raw_curve > 1000:
            # Sharp curve
            curve_radius = 200 + ((raw_curve - 1000) / 1000 * 200)  # 200-400 range
        else:
            # Very sharp curve
            curve_radius = 100 + (raw_curve / 1000 * 100)  # 100-200 range
        
        # Ensure it stays within bounds
        curve_radius = max(100, min(999, int(curve_radius)))
            
        curvature_msg = "Curvature = {:.0f} m".format(curve_radius)
        
        # Get image width for positioning on the right side
        img_width = out_img.shape[1]
        
        if direction == 'L':
            y, x = self.left_curve_img[:,:,3].nonzero()
            out_img[y, x-100+img_width-W//2] = self.left_curve_img[y, x, :3]
            msg = "Left Curve Ahead"
        if direction == 'R':
            y, x = self.right_curve_img[:,:,3].nonzero()
            out_img[y, x-100+img_width-W//2] = self.right_curve_img[y, x, :3]
            msg = "Right Curve Ahead"
        if direction == 'F':
            y, x = self.keep_straight_img[:,:,3].nonzero()
            out_img[y, x-100+img_width-W//2] = self.keep_straight_img[y, x, :3]

        # Enhanced text styling with better colors and background - positioned on RIGHT
        text_x_base = img_width - W + 10  # Right side with margin
        cv2.putText(out_img, msg, org=(text_x_base, 240), 
                   fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1.1, 
                   color=(255, 255, 255), thickness=2)
        if direction in 'LR':
            # Add semi-transparent background for curvature text
            cv2.putText(out_img, curvature_msg, org=(text_x_base, 280), 
                       fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.9, 
                       color=(0, 255, 255), thickness=2)

        # Display "Correct Lane Keep Going" text on two lines - on RIGHT
        cv2.putText(
            out_img,
            "Correct Lane",
            org=(text_x_base, 380),
